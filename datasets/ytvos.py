    ...
    def prepare_metas(self):
        ...
        for vid in self.videos:
            ...
            if vid_len < self.num_frames:
                ...
                print(f"Skipping video {vid} (length {vid_len}) because it's shorter than required {self.num_frames} frames.") # Optional print for skipped videos
                skipped_count += 1
                continue # Skip processing this video if it's too short
...
def build(image_set, args):
    ...
    PATHS = {
        ...
        "vae": (root / "train", root / "meta_expressions" / "train" / "meta_expressions.json"),
        "val": (root / "valid", root / "meta_expressions" / "val" / "meta_expressions.json"),    # not used actually
    }
    img_folder, ann_file = PATHS[image_set]
    dataset = YTVOSDataset(args, img_folder, ann_file, transforms=make_coco_transforms(image_set, max_size=args.max_size, resize=args.augm_resize), return_masks=args.masks,
