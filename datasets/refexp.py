...
def make_coco_transforms(image_set, cautious, max_size):

    normalize = T.Compose([T.ToTensor(), T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    scales = [384, 432, 480, 512, 544, 576, 608, 640, 672, 720]
    final_scales = [360, 392, 416, 448, 480, 512, 544, 576]
                ...
                T.RandomSelect(
                    ...
                    T.Compose(
                        [
                            ...
                            T.RandomResize(final_scales, max_size=max_size),
                        ]
                    ),
                ),
                normalize,
...
def build(dataset_file, image_set, args):
    ...
    PATHS = {
        ...
        "vae": (root / "train2014", root / dataset / f"{mode}_{dataset}_train.json"),
    ...
    dataset = ModulatedDetection(
        img_folder,
        ann_file,
        transforms=make_coco_transforms(image_set, False, args.max_size),
