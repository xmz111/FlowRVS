...
class ModulatedDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            ####### filter frame with extre h w ratio
            init_h, init_w, _ = np.asarray(img).shape
            min_size = min(init_w, init_h) / (max(init_w, init_h) / 640)
            if min_size < 200:
                print("Size of {} is too small, continue.".format(idx))
            ...
            # <PIL.Image.Image image mode=RGB size=612x612 at 0x7F1BCAF03E20>  masks: torch.Size([1, 361, 640]) tensor([0, 1], dtype=torch.uint8)
            # dict_keys(['boxes', 'labels', 'caption', 'masks', 'image_id', 'area', 'iscrowd', 'valid', 'orig_size', 'size'])
            img, target = self.prepare(img, target)

            # for a image, we rotate it to form a clip
            seq_images, seq_instance_masks = [img], [target['masks'].numpy()]
            # print(np.array(seq_images[0]).shape, seq_instance_masks[0].shape, 'aaaaaa') # (427, 640, 3) (1, 427, 640)
            numpy_masks = target['masks'].numpy()  # [1, H, W]

            numinst = len(numpy_masks)
            assert numinst == 1
            # augment to create sequence
            ...
            # FIXME: handle "valid", maybe already fixes, i.e., some valid is 0
            # detect with frame does not contain box
        ...
        # add nouns
        # tokens = nltk.wordpunct_tokenize(target["caption"])
        # nouns = ";".join([w for (w, pos) in nltk.pos_tag(tokens) if self.is_noun(pos)])
        # target['nouns']=nouns
...
def make_coco_transforms(image_set, max_size):
    ...
    if image_set == 'train':
        return T.Compose([
            ...
            T.RandomSelect(  # Is that needed? .... random select one
                T.Compose([
...
def build(dataset_file, image_set, args):
    ...
    dataset = ModulatedDetection(
        ...
        transforms=make_coco_transforms(image_set, args.max_size),  # 640
